class Admin::TooltipsController < Admin::AdminController
  ...
  skip_before_filter :check_admin, :only => :tooltip_content
  ...
  def create
    ...
    respond_to do |format|
      if @tooltip.save
        format.html { redirect_to([:admin,@tooltip], :notice => 'Tooltip was successfully created.') }
        format.xml  { render :xml => @tooltip, :status => :created, :location => [:admin,@tooltip] }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @tooltip.update_attributes(params[:tooltip])
        format.html { redirect_to([:admin,@tooltip], :notice => 'Tooltip was successfully updated.') }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_tooltips_url) }
      format.xml  { head :ok }
    end
  end
