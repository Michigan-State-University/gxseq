      ...
      unless params[:jrws].blank?
         ...
         case jrws['method']
            ...
            when 'describe'
              ...
              begin
                begin
                  @gene_model = GeneModel.find_by_id(param['id']) || Seqfeature.find(param['id']).gene_model
                end
         ...
         if(params[:annoj_action] == 'lookup')
             ...
             if(params[:query] && !params[:query].blank?)
               ...
               qualifiers = SeqfeatureQualifierValue.limit(1000).includes([:term,:seqfeature]).where{seqfeature.bioentry_id.in bioentry_ids}.where{seqfeature.display_name.in GeneModel.seqfeature_types }.where{term.name.not_in(Seqfeature.excluded_search_terms)}.where{upper(value)=~"%#{query}%"}
                  ...
                  ["gene","cds","mrna"].each do |feature|
                      if fea = gene_model.send(feature)
                          fea.qualifiers.each do |q|
                              ...
                              next if(Seqfeature.excluded_search_terms.include?(q.term.name))
                              # avoid repeats
