class ChipSeq < Experiment  
  ...
  def load_asset_data
    ...
    begin
      ...
      logger.error("#{Time.now} \n #{$!}")
      puts "Error: failed to compute peaks #{Time.now}"
    end      
  ...
  def set_peaks(bioentry_id, data)
    ...
    begin
      Experiment.transaction do
        ...
        data.each do |d|
          unless (d.size == 4 && d[0].kind_of?(Integer) && d[1].kind_of?(Integer) && d[2].respond_to?("to_i") && (d[2].to_i > 0)  && d[3].kind_of?(Integer));e= "Error invalid format #{d.inspect}";puts e;logger.info "\n\n#{e}\n\n";next;end
          unless d[0]<d[1];e= "Error start > end";puts e;logger.info "\n\n#{e}\n\n";next;end
          unless (d[0]>0 && d[3] < bc) && (d[0]<=d[3] &&d[3]<=d[1]);e= "Error peak out of bounds #{d.inspect}";puts e;logger.info "\n\n#{e}\n\n";next;end
          self.peaks.create(:start_pos => d[0],:end_pos => d[1],:val => d[2],:pos => d[3],:bioentry_id => bioentry_id)
        end
      end
    rescue
  ...
  #
  # def formatted_data(num=200)
  #   bc = base_counts
  #   data="["
  #   summary_data(num).each_with_index do |n,idx|
  #     data+="\n\t['#{(idx*(bc/num)).ceil}',#{n}],"
  #   end
  #   data.chop!
  #   data += "]"
  ...
  #
  # def column_format
  #   a=[['string','Position'],['number','Count']]
  #   if peaks.size > 0
  #     a<<['number','Peak']
  #   end
  # end
