class Groups < Thor
  ...
  def list
    ...
    puts "-\t-\tID\tName#{' '*(max_name_count-4)}Owner\temail\tTaxonVersionCount\tSampleCount"
    groups.each_with_index do |group,idx|
      puts "\t#{idx})\t#{group.id}\t#{group.name}#{' '*(max_name_count-group.name.length)}#{group.owner.login}\t#{group.owner.email}\t#{group.taxon_versions.count}\t#{group.experiments.count}"
    end
  end
end