      ...
      def construct_sql
        case
          ...
          when @reflection.options[:as]
            @finder_sql =
              #NT 8-9-11 Fix for polymorhpic table, uses string column ('id1,id2') for item_id instead of integer
              #"#{@reflection.quoted_table_name}.#{@reflection.options[:as]}_id = #{owner_quoted_id} AND " +
              "#{@reflection.quoted_table_name}.#{@reflection.options[:as]}_id = '#{@owner.id.to_s}' AND " +
